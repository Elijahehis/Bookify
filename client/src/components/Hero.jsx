...
const Hero = () => {
    ...
    const onsSearch = async (e) => {
        e.preventDefault();
        navigate(`/rooms?destination=${destination}`)
        await axios.post("/api/user/store-recent-serach", { recentSearchCity: destination }, { headers: { Authorization: `Bearer ${await getToken()}` } })
    ...
    return (
        <div className="flex flex-col items-start justify-center px-6 md:px-16 lg:px-24 xl:px-32
         text-white bg-[url('/src/assets/heroImage.png')] bg-no-repeat bg-cover bg-center h-screen" >
            ...
            <form onSubmit={onsSearch} className='bg-white text-gray-500 rounded-lg px-6 py-4 mt-8 flex flex-col md:flex-row max-md:items-start gap-4 max-md:mx-auto'>
